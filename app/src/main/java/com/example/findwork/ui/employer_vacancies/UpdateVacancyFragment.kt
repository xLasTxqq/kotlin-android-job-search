...
import com.example.findwork.databinding.FragmentUpdateVacancyBinding
...
class UpdateVacancyFragment : Fragment() {
    ...
    private val dataModelNewVacancy: UpdateVacancyViewModel by activityViewModels()
    private lateinit var binding: FragmentUpdateVacancyBinding
    ...
    override fun onCreateView(
        ...
        binding = FragmentUpdateVacancyBinding.inflate(inflater , container , false)
...
//            else JSONObject(request as Map<* , *>))
            updateBodyRequest()
            dataModelNewVacancy.createVacancy(Request.Method.POST , url , dataModelNewVacancy.request.value!!)
...
//                    }
    ...
    private fun updateBodyRequest() {
        ...
        if (binding.inputDescription.text.isNotEmpty()) request["description"]=binding.inputDescription.text.toString()
                else request.remove("description")
        if (binding.editContactName.text.isNotEmpty()) request["contacts_name"]=binding.editContactName.text.toString()
                else request.remove("contacts_name")
        if (binding.editContactEmail.text.isNotEmpty()) request["contacts_email"]=binding.editContactEmail.text.toString()
                else request.remove("contacts_email")
        if (binding.editContactPhone.text.isNotEmpty()) request["contacts_phones"]=binding.editContactPhone.text.toString()
                else request.remove("contacts_phones")
        if (binding.editTextSalaryTo.text.isNotEmpty()) request["salary_to"]=binding.editTextSalaryTo.text.toString()
                else request.remove("salary_to")
        if (binding.editTextSalaryFrom.text.isNotEmpty()) request["salary_from"]=binding.editTextSalaryFrom.text.toString()
    ...
    private fun validate() {
        binding.apply {
            if (inputName.text.isBlank()) {
                ...
                && editTextSalaryTo.text.toString().toDouble()<editTextSalaryFrom.text.toString().toDouble()) {
                ...
                binding.submit.isEnabled = false
            } else if (editContactEmail.text.isNotBlank() && !Patterns.EMAIL_ADDRESS.matcher(editContactEmail.text).matches()) {
                editContactEmail.error = "Не валидная почта"
                binding.submit.isEnabled = false
            } else if (editContactPhone.text.isNotBlank() && !Patterns.PHONE.matcher(editContactPhone.text).matches()) {
